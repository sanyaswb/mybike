...
function valuesTopBar() {
  // Безпечна зчитка — щоб уникнути помилок, якщо DOM елементи не знайдені
  if (
    !compareBarFirst ||
    !compareBarLast ||
    !firstBarProcent ||
    ...
    console.warn("Bars or procent nodes are missing");
    return;
  }

  const allProcent = firstBikeScore + lastBikeScore;

  // Якщо сумарний рахунок 0 -> нічого не підрахувати. Встановимо нейтральний вигляд (50/50).
  let procentFirstBar;
  let procentLastBar;

  if (allProcent === 0 || !isFinite(allProcent)) {
    procentFirstBar = 50;
    procentLastBar = 50;
  } else {
    procentFirstBar = Math.round((firstBikeScore / allProcent) * 100);
    procentLastBar = 100 - procentFirstBar; // гарантує, що сума = 100
  }

  // Відображення тексту (порожній рядок замість <p></p> для чистоти)
  // Якщо один з показників 100% — показуємо тільки його
  if (procentFirstBar === 100 && procentLastBar === 0) {
    firstBarProcent.innerHTML = `<p>${procentFirstBar}%</p>`;
    lastBarProcent.innerHTML = `<p></p>`;
  } else if (procentLastBar === 100 && procentFirstBar === 0) {
    lastBarProcent.innerHTML = `<p>${procentLastBar}%</p>`;
    firstBarProcent.innerHTML = `<p></p>`;
  } else {
    firstBarProcent.innerHTML = `<p>${procentFirstBar}%</p>`;
    lastBarProcent.innerHTML = `<p>${procentLastBar}%</p>`;
  }

  // Встановлюємо ширини стовпчиків
  ...
  // Колір заливки залежно від того, хто лідирує
  ...
  // Очистка для наступного підрахунку (зберігаємо поточну поведінку)
  firstBikeScore = 0;
  lastBikeScore = 0;
  weightsFirstBike = {};
  weightsLastBike = {};
...
function renderCompareTable() {
  ...
  const maxWeights = getProductsMaxWeights();
  ...
  function normalizeValue(value) {
    ...
    if (typeof value === "string") {
      ...
      if (weights[value] !== undefined) return weights[value];
    }
    return null; // нечислові значення
  }

  let html = '<ul class="compare__tth-list">';

  for (const prop of propertiesToCompare) {
    const key = prop.key;
    const label = prop.label;

    const firstValue = selectedFirstBike[key] ?? "---";
    const lastValue = selectedLastBike[key] ?? "---";

    const firstNumeric = normalizeValue(firstValue);
    const lastNumeric = normalizeValue(lastValue);
    ...
    if (
      ...
      if (key === "weight") {
        const maxWeight = Math.max(
          ...Object.values(products).map((b) => b.weight)
        );
        leftWidth = ((maxWeight - firstNumeric) / maxWeight) * 100;
        rightWidth = ((maxWeight - lastNumeric) / maxWeight) * 100;
      } else {
        leftWidth = (firstNumeric / maxWeights[key]) * 100;
        rightWidth = (lastNumeric / maxWeights[key]) * 100;
      }

      leftWidth = Math.min(100, Math.max(0, leftWidth));
      rightWidth = Math.min(100, Math.max(0, rightWidth));
    }

    html += `
      <div class="compare__tth-weights compare__tth-weights--left compare__tth-weights--right ${
        hasHisto ? "compare__tth-weights--has-histo" : ""
      }"
      style="--left-width:${leftWidth}%; --right-width:${rightWidth}%;">
        <li class="compare__tth-item">
          <p class="compare__tth-text compare__tth--first">${firstValue}</p>
          <p class="compare__tth-text compare__tth--value">${label}</p>
          <p class="compare__tth-text compare__tth--last">${lastValue}</p>
        </li>
      </div>`;
  }

  html += "</ul>";
  compareCardTth.innerHTML = html;
...
compareLists.addEventListener("click", (event) => {
  ...
  // helper: перетворити значення (може бути number, "$ 123", або ключ із weights)
  function toNumericIfPossible(val) {
    if (typeof val === "number") return val;
    if (typeof val === "string") {
      // спробуємо парсити число з рядка (наприклад "$ 2549" або "2549")
      const num = parseFloat(String(val).replace(/[^0-9.]/g, ""));
      if (!isNaN(num)) return num;
      // якщо в мапі weights є відповідник — повертаємо його
      if (weights[val] !== undefined) return weights[val];
    }
    return null; // нечислові/неможливо перетворити
  }

  if (slot === "first") {
    selectedFirstBike = { ...bikeData };
    // створюємо копію для ваг, але не перезаписуємо оригінальні текстові поля
    ...
    // Пройдемося по ключах і замінимо лише ті, що мапляться в числа
    for (const key in weightsFirstBike) {
      const numeric = toNumericIfPossible(selectedFirstBike[key]);
      if (numeric !== null) {
        weightsFirstBike[key] = numeric;
      } else {
        // залишаємо як є (рядок або undefined), щоб render/show використовував оригінал
        weightsFirstBike[key] = selectedFirstBike[key];
      }
    }

    // оновлюємо UI таблиці перед підрахунком переможця
    renderCompareTable();
    whoWin();
  } else if (slot === "last") {
    ...
    for (const key in weightsLastBike) {
      const numeric = toNumericIfPossible(selectedLastBike[key]);
      if (numeric !== null) {
        weightsLastBike[key] = numeric;
      } else {
        weightsLastBike[key] = selectedLastBike[key];
      }
    }
